...
class Cliente {
    ...
    public function obtenerClientes() {
        $query = "SELECT c.id_cliente, c.nombre, c.apellido, c.email, c.edad,
                    i.plan, i.pack, i.duracion
                  FROM clientes c
                  JOIN inscripcion i
                  ON c.id_cliente = i.id_cliente";
        $resultado = $this->conexion->conexion->query($query); //Ejecutamos la consulta

        $clientes = []; //Arreglo para almacenar los resultados
        while ($fila = $resultado->fetch_assoc()) { //Recorre los resultados para 
            $clientes[] = $fila; //Agrega cada fila al arreglo
        }
        return $clientes; //Devuelve los resultados de los clientes
    }
    ...
    public function eliminarCliente($id_cliente) {
        try {
            ...
            if (!$stmtInscripcion->execute()) {
                throw new Exception("Error al eliminar la inscripción: " . $stmtInscripcion->error);
            }
            ...
            if (!$stmtCliente->execute()) {
                throw new Exception("Error al eliminar al cliente: " . $stmtCliente->error);
            }
