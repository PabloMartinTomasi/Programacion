...
def registrar_cliente_inscripcion():
    id_cliente = int(input("Ingresa el id del cliente, para poderlo inscribir:"))
    id_actividad = int(input("Ingrese el id de la especialidad a la que se quiere inscribir el cliente:"))
    
    nuevo_inscripcion = (id_cliente, id_actividad)
    consulta_inscripcion = """INSERT INTO inscripciones (id_cliente, id_actividad) VALUES (%s, %s)"""
    cursor.execute(consulta_inscripcion,  nuevo_inscripcion)
    conexion.commit()
    id_inscripcion = cursor.lastrowid
    print(Fore.GREEN+ f"[Mensaje]: Inscripción registrada exitosamente. ")
    print(Style.RESET_ALL)
...
def eliminar_entrenador():
    id_inscripcion = int(input("Introduce el id de la inscripcion que deseas eliminar:"))
    consulta_eliminar = """DELETE FROM inscripciones WHERE id_inscripcion = %s"""
    cursor.execute(consulta_eliminar, (id_inscripcion,))
    conexion.commit()
    print(Fore.RED+ f"[Mensaje de confirmación] La inscripcion con el id {id_inscripcion} ha sido eliminado.")
    print(Style.RESET_ALL)
